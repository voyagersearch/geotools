...
public class JDBCDataStoreTest {
    ...
    public void testReaderCallback() throws Exception {
        ...
        verify(callback, times(1)).init();
        verify(callback, times(4)).beforeRow();
        verify(callback, times(3)).afterRow(true);
        verify(callback, times(1)).afterRow(false);
        verify(callback, times(1)).finish();
    }
}
